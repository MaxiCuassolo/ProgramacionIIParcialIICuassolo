...
try {
    ...
    SELECT Id AS id, Nombre AS nombre, Categoria AS categoria, Duracion AS duracion, CuposDisponibles AS cupos, Activo AS activo
    FROM Cursos
    ORDER BY Id DESC
    `);
...
async function registrarCurso(req, res) {
try {
    const { nombre, categoria, duracion, cupos, activo } = req.body;

    if (!nombre || !categoria || !duracion === undefined || !cupos === undefined || activo === undefined) {
    ...
    .input("cupos", sql.Int, Number(cupos))
    .input("activo", sql.Bit, activo === true || activo === "true")
    .query(`
        INSERT INTO Cursos (Nombre, Categoria, Duracion, Cupos, Activo)
        OUTPUT INSERTED.Id AS id, INSERTED.Nombre AS nombre, INSERTED.Categoria AS categoria,
        INSERTED.Duracion AS duracion, INSERTED.CuposDisponibles AS cupos, INSERTED.Activo AS activo
        VALUES (@nombre, @categoria, @duracion, @cupos, @activo)
    `);

    res.status(201).json({ mensaje: "Curso registrado correctamente", curso: resultado.recordset[0] });
} catch (error) {
    res.status(500).json({ mensaje: "Error al registrar curso", error: error.message });
}
}
...
module.exports = { probarConexion, obtenerCursos, registrarCurso, eliminarCurso };
